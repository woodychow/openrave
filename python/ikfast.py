...
class IKFastSolver(AutoReloader):
    ...
    def _dec_solutionStackCounter(self):
        self._solutionStackCounter -= 1
    ...
    def checkForDivideByZero(self, eq, isPrimitive = True):
        """
        ...
        (3) In M3 of solveSingleVarible, we write
              atan2(sign(dummy)*s, sign(dummy)*c) 
            = atan2(s,c) + pi/2*(1-sign(dummy))   with dummy!=0
            so we need dummy in checkforzeros as well.

        Called by ComputeSolutionComplexity and solveSingleVariable only.
        """
        ...
        try:
            if eq.is_Function:
                if eq.func == sign:
                    pass
                    # assert(0) if sign is not used with atan2
                    # checkforzeros.append(eq.args[0]) if we add dummy in checkforzeros
                elif eq.func == atan2:
            ...
            checkforzeros += self.jointlists([self.checkForDivideByZero(arg, False) \
                                         for arg in eq.args \
                                         if not arg.is_number])
    ...
    def checkpow(self, expr, sexprs, unsolvedvars):
        """
        Used to be a sub-function inside ComputeSolutionComplexity. Refactored out.
        """
        if expr in self.checkpow_expr_dict:
            ...
            if expr.is_Pow:
                ...
                if expr.exp.is_number and expr.exp < 0:
                    score += 110000 if expr.base.has(*unsolvedvars) else 10000
    ...
    def ComputeSolutionComplexity(self, sol, solvedvars, unsolvedvars):
        """
        Checks for all solutions if there are divide-by-zero situations and fills checkforzeros for the solution.

        Only derives values for sol.checkforzeros and sol.score
        """
...
#                 print 'failed to rotate joint correctly',e
    ...
    def _ExtractTranslationsOutsideOfMatrixMultiplication(self, Links, LinksInv, solvejointvars):
        ...
        """
        # (deep) copy into a new list, but the sympy Matrices still have the same ID
        NewLinks = list(Links)
        assert(id(NewLinks)    != id(Links))
        assert(id(NewLinks[0]) == id(Links[0]))
        ...
        # Use LinksInv. Note in Links and LinksInv, orders are reverse, i.e.
        # Links = [ A_1, A_2, ..., A_p ] and LinksInv = [ inv(A_p), ... , inv(A_2), inv(A_1) ]
        Trighttrans[0:3,3] = -LinksInv[0][0:3,0:3] * LinksInv[1][0:3,3] - LinksInv[0][0:3,3]
        for j in range(3):
            if Trighttrans[j,3].has(*solvejointvars):
    ...
    def solve6DIntersectingAxes(self, T0links, T1links, transvars, rotvars, solveRotationFirst, endbranchtree):
        ...
        # solve for translation variables and verify
        log.info('Solving for translation variables: %r' % curvars)
        transtree = self.SolveAllEquations(AllEquations, \
                                           curvars = curvars, \
        ...
        try:
            ...
            othersolvedvars = self.freejointvars if solveRotationFirst else \
                              transvars + self.freejointvars
            AllEquations = self.buildEquationsFromRotation(T0links, Matrix(3,3,Ree), rotvars, othersolvedvars)
            currotvars = rotvars[:]

            # check, solve for rotation variables
            self.checkSolvability(AllEquations, rotvars, othersolvedvars)
            log.info('Solving for rotation variables: %r' % currotvars)
    ...
    def buildEquationsFromTwoSides(self, leftside, rightside, usedvars, \
                                   uselength = True):
        """
...
#                 continue
    ...
    def solveManochaCanny(self, \
        ...
        for ileftvar, leftvar in enumerate(dummys):
            try:
                exportcoeffeqs, exportmonoms = self.solveDialytically(newreducedeqs, \
                                                                      ileftvar, \
                                                                      getsubs = getsubs)
                break
            except self.CannotSolveError,e:
        ...
        if haszeroequations:
            ...
            for curvar in usedvars[:-1]:
                try:
                    ...
                    halfanglesolution = self.solvePairVariablesHalfAngle(raweqns = [eq.subs(curvarsubs) for eq in AllEquations], \
                                                                         var0 = unknownvars[0], \
                                                                         var1 = unknownvars[1], \
                                                                         othersolvedvars = self.freejointvars+[curvar])[0]
                    # sometimes halfanglesolution can evaluate to all zeros (katana arm)
                    # need to catch this and go to a different branch
                    ...
                    try:
                        ...
                        jointtrees2 += self.SolveAllEquations(AllEquationsOrig, \
                                                              ...
                                                              currentcases = currentcases, \
                                                              currentcasesubs = currentcasesubs)
                        log.info('[SOLVE %i] solveLiWoernleHiller returns solution for %r', \
                                 self._solutionStackCounter-1, curvars)
                    ...
                    jointtrees3 = []
                    log.info('[SOLVE %i] solveLiWoernleHiller calls SolveAllEquations for %r', \
                             self._solutionStackCounter-1, unknownvars)
                    treesecond = self.SolveAllEquations(AllEquations, \
                                                        curvars = unknownvars, \
                                                        othersolvedvars = self.freejointvars+[curvar,halfanglevar], \
                                                        solsubs = self.freevarsubs+curvarsubs+self.getVariable(halfanglevar).subs, \
                    ...
                    if AUinv is not None:
                        ...
                        for i in [0,2]:
                            ...
                            try:
                                ...
                                jointtrees3.append(AST.SolverSolution(jointname, \
                                                                      jointeval = [atan2(X[indices[i+1]], \
                                                                                         X[indices[i]])], \
                        ...
                        jointtrees3.append(AST.SolverCheckZeros('sanitycheck', \
                                                                jointcheckeqs, \
                                                                zerobranch = endbranchtree, \
                                                                nonzerobranch = [AST.SolverBreak('sanitycheck for solveLiWoernleHiller')], \
                                                                anycondition = False, \
                                                                thresh = 0.001))
            ...
            try:
                ...
                self._inc_solutionStackCounter()
                if len(raweqns) > 1:
                    log.info('[SOLVE %i] solveLiWoernleHiller calls solvePairVariablesHalfAngle for %r, %r', \
                             self._solutionStackCounter-1, usedvars[0], usedvars[1])
                    halfanglesolution = self.solvePairVariablesHalfAngle(raweqns = raweqns, \
                                                                         var0 = usedvars[0], \
                                                                         var1 = usedvars[1], \
                                                                         othersolvedvars = self.freejointvars)[0]
                    halfanglevar, unknownvar = (usedvars[0], usedvars[1]) if halfanglesolution.jointname==usedvars[0].name \
                                               else (usedvars[1], usedvars[0])
                    log.info('[SOLVE %i] solveLiWoernleHiller calls SolveAllEquations for %r', \
                             self._solutionStackCounter-1, [unknownvar])
                    nexttree = self.SolveAllEquations(raweqns, \
                                                      curvars = [unknownvar], \
                                                      othersolvedvars = self.freejointvars+[halfanglevar], \
                                                      solsubs = self.freevarsubs+self.getVariable(halfanglevar).subs, \
                    ...
                    try:
                        log.info('[SOLVE %i] solveLiWoernleHiller calls SolveAllEquations for %r', \
                                 self._solutionStackCounter-1, usedvars[2:])
                        finaltree = self.SolveAllEquations(AllEquations, \
                                                           curvars = usedvars[2:], \
                                                           othersolvedvars = self.freejointvars+usedvars[0:2], \
                                                           solsubs = self.freevarsubs+self.getVariable(usedvars[0]).subs+self.getVariable(usedvars[1]).subs, \
                                                           endbranchtree = endbranchtree, \
                                                           canguessvars = False, \
                                                           currentcases = currentcases, \
                                                           currentcasesubs = currentcasesubs)
                        ...
                        log.info('[SOLVE %i] solveLiWoernleHiller returns solution for %r', \
                                 self._solutionStackCounter-1, usedvars)
                        return preprocesssolutiontree+[halfanglesolution]+nexttree, usedvars
                    
                    except self.CannotSolveError, e:
                        log.info('[SOLVE %i] Cannot use SolveAllEquations for %r; ' + \
                                 'solveLiWoernleHiller returns solution for %r', \
                                 self._solutionStackCounter-1, usedvars[2:], usedvars[0:2])
                        jointtrees += endbranchtree
                        # sometimes the last variable cannot be solved,
                        # so returned the already solved variables and let the higher function take care of it 
...
[[[3,  7], [6,  4],  2],
 ...
 [[2, 10], [5,  9], 24]]
    ...
    def SolveAllEquations(self, \
                          ...
                          canguessvars    = True):
        """
        Use the following methods to attempt solving for 1 or 2 variables from curvars at a time.

        (1) Solve (>=1) equations for one variable (involving jx, sjx, cjx, htjx) using solveSingleVariable.

        (2) When there are exactly 2 variables, we assume they are aligned and solve (>=2) equations
            using solveSingleVariable with an introduced variable dummy.

        (3) Solve (>=2) equations for a pair of non-aligned variables using either
        - [a] solvePrismaticHingePairVariables, if one prismatic and one hinge, or
        - [b] solvePairVariables, if both are hinge.

        (4) Solve 1 equation for one variable jx using solveHighDegreeEquationsHalfAngle.

        (5) Conjecture there is a free variable and solve it using GuessValuesAndSolveEquations.

        If method (1) finds a solution that uniquely determines a variable, then use AddSolution to start 
        recursion immediately. Otherwise try methods (2) and (3), and if there is some solution found, call
        AddSolution to start recursion. Finally try methods (4) and (5).

        Primitive calls are by solveManochaCanny, solveLiWoernleHiller, solveKohliOsvatic, 
        and those solveFullIK_* functions.

        Recursive calls are by AddSolution and GuessValuesAndSolveEquations.
        """

        progress = 0.45 - 0.3/(1+self._scopecounter/100) # range is  [0.15, 0.45]
        ...
        for curvar in curvars:
            ...
            if len(raweqns) > 0:
                try:
                    ...
                    log.info('[SOLVE %i] Cannot use solveSingleVariable to solve for %r: %s', \
                             self._solutionStackCounter, curvar, e)
                finally:
                    self._dec_solutionStackCounter()
        ...
        # (TGN's comments)
        # If a single-variable solution has exactly 1 solution that uniquely determines this variable,
            ...
            for eq in raweqns:
                ...
                if neweq.has(dummyvar):
                    if neweq.has(*(othervars + curvars)):
                        ...
                        # break; don't know why breaking here ...
            ...
            if len(NewEquations) >= 2:
                ...
                try:
                    ...
                    log.info('[SOLVE %i] Cannot use solveSingleVariables for %r: %s', \
                             self._solutionStackCounter, dummyvar, e)
                finally:
                    self._dec_solutionStackCounter()
                ...
                if any([s.numsolutions()==1 for s in dummysolutions]):
                    ...
                    for dummysolution in dummysolutions:
                        ...
                        newsubs = [( value,  sin(dummyvar)) for value in sindummyvarsols] + \
                                  ...
                                  [(-value, -cos(dummyvar)) for value in cosdummyvarsols]
                        allzeros = all([trigsimp(eq.subs(newsubs)) == S.Zero for eq in NewEquationsAll])
                    ...
                    if len(solutions) > 0:
                        try:
                            log.info('[SOLVE %i] SolveAllEquations calls AddSolution for %r', \
                                     self._solutionStackCounter, [curvars[0]])
                            self._inc_solutionStackCounter()
                            tree = self.AddSolution(solutions, raweqns, [curvars[0]], \
                                                    othersolvedvars + [curvars[1]], \
                    ...
                    log.warn('Almost found two axes, but number of solutions is %r', \
                             [s.numsolutions() for s in dummysolutions])
                    
        ## (3a) Solve (>=2) equations for a pair of one prismatic variable and one hinge variable using
        #       solvePrismaticHingePairVariables
        for var0, var1, raweqns, complexity in curvarsubssol:
            # check before calling solvePrismaticHingePairVariables
            if (self.IsPrismatic(var0.name) and self.IsHinge(var1.name)) or \
               (self.IsPrismatic(var1.name) and self.IsHinge(var0.name)):
                try:
                    log.info('[SOLVE %i] SolveAllEquations calls solvePrismaticHingePairVariables for %r, %r', \
                    ...
                    log.info('[SOLVE %i] Cannot use solvePrismaticHingePairVariables to solve for %r, %r: %s', \
                             self._solutionStackCounter, var0, var1, e)
                    pass
                finally:
                    self._dec_solutionStackCounter()
                    
        ## (3b) Solve (>=2) equations for a pair of hinge variables using solvePairVariables           
        for var0, var1, raweqns, complexity in curvarsubssol:
            try:
                log.info('[SOLVE %i] SolveAllEquations calls solvePairVariables for %r, %r', \
                         self._solutionStackCounter, var0, var1)
                self._inc_solutionStackCounter()
                ...
                log.info('[SOLVE %i] Cannot use solvePairVariables to solve for %r, %r', \
                         self._solutionStackCounter, var0, var1)
                log.debug(e)
                rawsolutions = []
            finally:
        ...
        # If so far there is some solution, call AddSolution to start recursion;
        ...
        if len(solutions) > 0:
            ...
            try:
                ...
                if self._isUnderAnalysis:
                    exec(ipython_str, globals(), locals())
                log.info('[SOLVE %i] Exit SolveAllEquations', self._solutionStackCounter-1)
                return prevbranch
            except self.CannotSolveError, e:
                raise self.CannotSolveError(e)
            finally:
        ...
        for curvar in curvars:
            ...
            for raweqn in raweqns:
                try:
                    log.info('[SOLVE %i] SolveAllEquations calls solveHighDegreeEquationsHalfAngle for %r', \
                             self._solutionStackCounter, curvar)
                    self._inc_solutionStackCounter()
        ...
        ## (5) Conjecture there is a free variable and solve for it using GuessValuesAndSolveEquations, only
        #      if either
        #      (a) one last joint is to be solved, or
        #      (b) CURRENTCASES is not empty and at least one joint is solved already.
        #      Otherwise, the case where no joints have been solved indicates ill-posed equations.
        if canguessvars and \
           ...
           (len(curvars) == 1 or (len(curvars) < len(self._solvejointvars) and \
            ...
            try:
                log.info('[SOLVE %i] SolveAllEquations calls GuessValuesAndSolveEquations for %r', \
                         self._solutionStackCounter, curvars)
                self._inc_solutionStackCounter()
                ...
                log.info('[SOLVE %i] Cannot use GuessValuesAndSolveEquations for %r: %s', \
                         self._solutionStackCounter, curvars, e)
            finally:
                self._dec_solutionStackCounter()
        
        # Have got this far, so perhaps two axes are aligned but cannot be detected?
    ...
    def _SubstituteGlobalSymbols(self, eq, globalsymbols = None):
        """
        Performs substitutions in EQ using GLOBALSYMBOLS and returns a (new if different) equation NEWEQ.
        """
        if globalsymbols is None:
            globalsymbols = self.globalsymbols
        neweq = eq.subs(globalsymbols)
    ...
    def GuessValuesAndSolveEquations(self, AllEquations, \
        ...
        scopecounter = int(self._scopecounter)
        hingevariables = [curvar for curvar in sorted(curvars, reverse = True) \
                          if self.IsHinge(curvar.name)]
        ...
        if len(curvars) == 1:
        ...
        elif len(curvars) >= 2 and len(hingevariables) > 0:
            ...
            if len(newtree) > 0:
                log.warn('c = %d; ' + \
                         ...
                         scopecounter, curvar)
                try:
                    log.info('[SOLVE %i] GuessValuesAndSolveEquations calls SolveAllEquations for %r', \
                             self._solutionStackCounter, leftovervars)
                    self._inc_solutionStackCounter()
                    ...
                    log.info('[SOLVE %i] Cannot use SolveAllEquations for %r: %s', \
                            self._solutionStackCounter, leftovervars, e)
                finally:
                    self._dec_solutionStackCounter()                    
    ...
    def solveHighDegreeEquationsHalfAngle(self, lineareqs, var, tosubs = []):
        ...
        Current approach is to return an AST.SolverPolynomialRoots object that contains the first feasible polynomial in htvar.

        Called by solvePairVariables, solveSingleVariable, (not sure if necessary) SolveAllEquations.
        """

        log.info('[SOLVE %i] Starting solveHighDegreeEquationsHalfAngle for %r', \
    ...
    def checkFactorPmI(coeffs, thresh = 1e-12):
        ...
        Called by checkFinalEquation only.
        """
    ...
    def solveSingleVariable(self, raweqns, \
        ...
        Use the following methods to attempt solving for VAR.
        (1) Solve a single equation in VAR or HTVAR only, where CVAR, SVAR do not occur.
            Returns solution for (1a) VAR; (1b) HTVAR if found.

        (2) Solve each combination of two equations in CVAR and SVAR.
            Each equation is either linear in CVAR and SVAR, or quadratic in one variable when the other does not occur.
            In most cases, atan2 and atan2check are used, and divide-by-zero cases appear.

        (3) Solve a single equation in CVAR, SVAR in the following forms:
            (3a) a*CVAR + b*SVAR + c. atan2 and asin are used.
            (3b) substitute CVAR for SVAR and solve for CVAR
            (3c) substitute SVAR for CVAR and solve for SVAR
            (3d) neither CVAR nor SVAR occurs and solve for VAR (1a???)
            (3e) replace CVAR, SVAR by HTVAR and use solveHighDegreeEquationsHalfAngle to solve for HTVAR

        (4) Call solveHighDegreeEquationsHalfAngle to solve the first feasible equation in the set of equations.

        Remarks: solveHighDegreeEquationsHalfAngle returns an AST.SolverPolynomialRoots object that contains 
                 the polynomial in HTVAR corresponding to the first feasible equation.
        """
        ...
        varsym = self.getVariable(var)
        cvar, svar, htvar = varsym.cvar, varsym.svar, varsym.htvar
        vars = [cvar, svar, htvar, var]
        othersubs = self.jointlists([self.getVariable(othersolvedvar).subs \
...
#                     eqns.append(eq)
        ...
        if len(eqns) == 0:
            raise self.CannotSolveError('Need at least 1 equation; now there is none')
        ...
        for eq in eqns:
                    ...
                    if len(jointsolutions) > 0 and \
                       all([self.isValidSolution(s) and s != S.Zero \
                        ...
                        if not hasdividebyzero:
                            log.info('[SOLVE %i] solveSingleVariable (M1a) returns a solution that solves %r for %r', \
                                     self._solutionStackCounter, eq, var)
                            return returnfirstsolutions
                        else:
                            log.info('[SOLVE %i] solveSingleVariable (M1a) finds a solution that solves %r for %r\n' + \
            ...
            numvar = self.countVariables(eqnew, htvar)
            if Poly(eqnew, htvar).TC() != S.Zero and numvar in [1, 2]:
                try:
                    ...
                    if len(jointsolutions) > 0 and \
                       all([self.isValidSolution(s) \
                        ...
                        hasdividebyzero = any([len(self.checkForDivideByZero(self._SubstituteGlobalSymbols(s))) > 0 \
                                               for s in jointsolutions])
                        if not hasdividebyzero:
                            log.info('[SOLVE %i] solveSingleVariable (M1b) returns a solution that solves %r for %r', \
                                     self._solutionStackCounter, eq, htvar)
                            return returnfirstsolutions
                        else:
                            log.info('[SOLVE %i] solveSingleVariable (M1b) finds a solution that solves %r for %r\n' + \
                                     ' '*18 + 'Has divide-by-zero conditions', \
                                     self._solutionStackCounter, eq, htvar)
                            # exec(ipython_str, globals(), locals())
        ...
        # finished eq for-loop using Method 1a & 1b
        if len(returnfirstsolutions) > 0:
            ...
            log.info('[SOLVE %i] solveSingleVariable (M1) returns a solution for %r\n' + \
                     ' '*18 + 'All solutions have divide-by-zero conditions', \
                     self._solutionStackCounter, var)
            return returnfirstsolutions

        ## (2) Solve a pair of equations in cvar, svar
        solutions = []
        if len(eqns) > 1:
            neweqns = []
            listsymbols = []
            symbolgen = cse_main.numbered_symbols('const')
            for eq in eqns:
                eq = eq.subs(varsym.subs)
                neweq, symbols = self.groupTerms(eq, [cvar, svar, var], symbolgen)
                try:
                    # remove coupled equations
                    if any([(m[0]>0) + (m[1]>0) + (m[2]>0) > 1 \
                            for m in Poly(neweq, cvar, svar, var).monoms()]):
                        continue
                except PolynomialError:
                    continue
                ...
                try:
                    ...
                    if Poly(neweq, svar).degree() > maxdegree or \
                       Poly(neweq, cvar).degree() > maxdegree:
                        log.debug('Degree > %i; ignore equation %r', maxdegree, neweq)
                        continue
                except PolynomialError:
                    continue
                
                try:
                    if Poly(neweq, svar).TC() == S.Zero or \
                       Poly(neweq, cvar).TC() == S.Zero or \
                       Poly(neweq,  var).TC() == S.Zero:
                        log.debug('%s allows trivial solution for %s, ignore', eq, varsym.name)
                        continue
                except PolynomialError:
                    continue
                
                rank = self.codeComplexity(neweq) + \
                       sum(self.codeComplexity(s[1]) for s in symbols)
                # each entry of NEWEQNS is (complexity, neweq)
                neweqns.append((rank, neweq))
                listsymbols += symbols
                
            # Need two equations for two variables: each entry of EQCOMBINATIONS is (complexity, [eq0==0, eq1==0])
            eqcombinations = [(eq0[0] + eq1[0], \
                               [Eq(eq0[1],0), Eq(eq1[1],0)]) \
                              for eq0, eq1 in combinations(neweqns, 2)]
            # sort pairs of equations
            eqcombinations.sort(lambda x, y: x[0]-y[0])
            hasgoodsolution = False

            for complexity, eqs in eqcombinations:
                # skip if too complex
                if len(solutions) > 0 and complexity > 200:
                    break
                # try to solve for both sin and cos terms
                if not (self.has(eqs, svar) and \
                        self.has(eqs, cvar)):
                    continue
                ...
                if sol is not None:
                    # sol is a dictionary if there is one solution, a list if there are more than one
                    sollist = ([(sol[svar], sol[cvar])] if \
                               sol.has_key(svar) and \
                               sol.has_key(cvar) else []) \
                               if hasattr(sol, 'has_key') else sol
                    ...
                    for svarsol, cvarsol in sollist:
                        svarsol_subbed = svarsol.subs(listsymbols)
                        cvarsol_subbed = cvarsol.subs(listsymbols)
                        
                        # solutions cannot be trivial
                        soldiff = svarsol_subbed - cvarsol_subbed
                        soldiffComplexity = self.codeComplexity(soldiff)
                        if soldiffComplexity < 1000 and soldiff.expand() == S.Zero:
                            # TGN: what does S.Zero mean? svar = cvar = +/- 1/sqrt(2)??
                            break
                        
                        svarComplexity = self.codeComplexity(svarsol_subbed)
                        cvarComplexity = self.codeComplexity(cvarsol_subbed)
                        
                        if svarComplexity < 600 and \
                           cvarComplexity < 600 and \
                           ((svarsol_subbed.expand() == S.Zero and \
                             Abs(cvarsol_subbed.expand()) != S.One) or \
                            (cvarsol_subbed.expand() == S.Zero and \
                             Abs(svarsol_subbed.expand()) != S.One)):
                        ...
                        svarnum, svardenom = fraction(svarsol_subbed)
                        cvarnum, cvardenom = fraction(cvarsol_subbed)
                        
                        if self.equal(svarnum, cvarnum) and \
                           self.equal(svardenom, cvardenom):
                            break
                        
                        if not (\
                                self.isValidSolution(svarnum) and \
                                self.isValidSolution(svardenom) and \
                                self.isValidSolution(cvarnum) and \
                                self.isValidSolution(cvardenom) ):
                            continue
                        
                        # check if there exists at least one test solution with non-zero denominators
                        testeqs = [svardenom.subs(tosubs).subs(othersubs), \
                                   cvardenom.subs(tosubs).subs(othersubs)]
                        ...
                        if not testsuccess:
                            continue
                        scomplexity = self.codeComplexity(svarnum) + \
                                      self.codeComplexity(svardenom)
                        ccomplexity = self.codeComplexity(cvarnum) + \
                                      self.codeComplexity(cvardenom)
                        ...
                        if scomplexity < 500 and len(str(svardenom)) < 600:
                            svardenom = simplify(svardenom)
                        if ccomplexity < 500 and len(str(cvardenom)) < 600:
                            cvardenom = simplify(cvardenom)
                        if self.chop(svardenom)== 0 or self.chop(cvardenom)== 0:
                            break
                        ...
                        svarfracsimp_denom = self.SimplifyTransform(self.trigsimp(svardenom))
                        cvarfracsimp_denom = self.SimplifyTransform(self.trigsimp(cvardenom))
                        ...
                        """
                               ...
                               if lcmsc == svarfracsimp_denom:
                                   cvarnum *= lcmsc/cvarfracsimp_denom
                                   cvarfracsimp_denom = lcmsc
                                   denomsequal = True
                               elif lcmsc == cvarfracsimp_denom:
                                   svarnum *= lcmsc/svarfracsimp_denom
                        ...
                        if denomsequal:
                            ...
                            log.debug('denom of %s = %s', var.name, svarfracsimp_denom)
                        else:
                            log.debug('denoms of %s''s svar and cvar are different: %r, %r', \
                                      var.name, svarfracsimp_denom, cvarfracsimp_denom)

                        svarsolsimp = self.SimplifyTransform(self.trigsimp(svarnum))#*denom)
                        cvarsolsimp = self.SimplifyTransform(self.trigsimp(cvarnum))#*denom)
                        ...
                        # which is more robust against NaNs
                        #
                        # Otherwise, we use atan2check(x,y) and CodeGenerator._WriteExprCode will check if
                        # Abs(x), Abs(y) <  IKFAST_ATAN2_MAGTHRESH, and
                        # Abs(x*x+y*y-1) <= IKFAST_SINCOS_THRESH
                        # which CANNOT hold at the same time!!
                        #
                        # TGN: what did it attempt to check in the first place???
                        ...
                        if denomsequal and not svarfracsimp_denom.is_number:
                            ...
                            #   atan2(sign(dummy)*s, sign(dummy)*c) 
                            # = atan2(s,c) + pi/2*(1-sign(dummy))   with dummy!=0
                            # TGN: then we will need to add dummy into checkforzeros; what to do?
                            expandedsol = atan2(svarsolsimp, cvarsolsimp) + \
                                          pi/2*(-S.One + sign(svarfracsimp_denom))
                            # expandedsol = atan2(svarsolsimp*svarfracsimp_denom, \
                            #                    cvarsolsimp*svarfracsimp_denom)
                        else:
                            svarsolsimp /= svarfracsimp_denom
                            cvarsolsimp /= cvarfracsimp_denom
                            
                            if svarsolsimp.is_number and \
                               cvarsolsimp.is_number and \
                               Abs(svarsolsimp**2 + cvarsolsimp**2 - S.One).evalf() > 1e-10:
                        ...
                        if len(unknownvars)>0:
                            unsolvedsymbols = self.jointlists([self.getVariable(unknownvar).vars \
                                                               for unknownvar in unknownvars if unknownvar != var])
                            solversolution.equationsused = [eq for eq in eqns \
                                                            if not eq.has(*unsolvedsymbols)] if len(unsolvedsymbols) > 0 else eqns
            ...
            if len(solutions) > 0 or hasgoodsolution:
                ...
                log.info('[SOLVE %i] solveSingleVariable (M2) returns a solution for %r using atan2', \
                         self._solutionStackCounter, var)
                return solutions

        ## (3) solve one equation at a time
        for eq in eqns:
            symbolgen = cse_main.numbered_symbols('const')
                ...
                if m is not None:
                    ...
                    if self.isValidSolution(constsol) and \
                       self.isValidSolution(asinsol):
                        ...
                        log.info('[SOLVE %i] solveSingleVariable (M3a) finds solution solving %r for %r using atan2 & asin', \
                                 self._solutionStackCounter, eq, var)
                ...
                try:
                    if numcvar == 1 or (numcvar == 2 and numsvar == 0):
                        ...
                        if len(jointsolutions) > 0:
                            log.info('[SOLVE %i] solveSingleVariable (M3b) finds solution solving %r for cos(%r)', \
                                     self._solutionStackCounter, eq, var)
                            solutions.append(AST.SolverSolution(var.name, \
            ...
            # (c) substitute svar for cvar and solve for svar
            if numsvar > 0:
                try:
                    if numsvar == 1 or (numsvar == 2 and numcvar == 0):
                        ...
                        if len(jointsolutions) > 0:
                            log.info('[SOLVE %i] solveSingleVariable (M3c) finds solution solving %r for sin(%r)', \
                                     self._solutionStackCounter, eq, var)                            
                            solutions.append(AST.SolverSolution(var.name,
            ...
            # (d) neither cvar nor svar occurs
            # Hasn't this been solved by M1???
            if numcvar == 0 and numsvar == 0:
                try:
                    ...
                    if all([self.isValidSolution(s) and s != S.Zero \
                        ...
                        log.info('[SOLVE %i] solveSingleVariable (M3d) finds solution solving %r for %r', \
                                 self._solutionStackCounter, eq, var)
                        solutions.append(AST.SolverSolution(var.name, \
                                                            jointeval = jointsolutions, \
                                                            isHinge = self.IsHinge(var.name)))
        ...
        if len(solutions) > 0:
            log.info('[SOLVE %i] solveSingleVariable (M3) returns %i solution(s) for %r', \
                     self._solutionStackCounter, len(solutions), var)
            return solutions

        ## (4) Use high degree solver for htvar
        try:
            log.info('[SOLVE %i] solveSingleVariable tries solveHighDegreeEquationsHalfAngle to solve %r for %r', \
                     self._solutionStackCounter, eqns, htvar)
            self._inc_solutionStackCounter()
            solution = self.solveHighDegreeEquationsHalfAngle(eqns, var)
        except self.CannotSolveError, e:
            log.info('[SOLVE %i] Cannot use solveHighDegreeEquationsHalfAngle to solve for %r: %s', \
                     self._solutionStackCounter, htvar, e)
            raise self.CannotSolveError(e)
        finally:
            self._dec_solutionStackCounter()
            
        log.info('[SOLVE %i] solveSingleVariable (M4) returns a solution of %r', \
                 self._solutionStackCounter, htvar)
        return [solution]
    ...
    def solvePrismaticHingePairVariables(self, raweqns, \
        ...
        Called by SolveAllEquations only.
        """
        log.info('[SOLVE %i] Starting solvePrismaticHingePairVariables for %r, %r', \
                 self._solutionStackCounter, var0, var1)

        eqns = [eqn for eqn in raweqns if eqn.has(var0, var1)]
        if len(eqns) < 2:
    ...
    def solvePairVariables(self, raweqns, var0, var1, \
                           ...
                           unknownvars = None):
        """
        Solves >=2 equations where only c0, s0, c1, s1 appear AND neither of var0, var1 appears. 
        Both variables are hinge variables; otherwise solvePrismaticHingePairVariables is called.

        Called by SolveAllEquations only.

        Step 1. Do the following to collect more equations and hope they will be useful in subsequent steps.
                (a) Eliminate higher (>1) powers of s0, s1 by substituting c0, c1 in.
                (b) Try taking linear combination of any pair of equations to eliminate pairwise variables,
                which are s0c1, s0s1, c0c1, c0s1, s0c0, s1c1.
                (c) Try solving a 4x4 linear system in pairwise variables, represented by s0, c0, s1, c1.
                (d) Check if there are equations in the form a*s0 + f(s1,c1) = 0 or a*c0 + f(s1,c1) = 0 and
                    vice versa. If not, try taking a linear combination of pair of equations that have 
                    cj*cj or cj*sj and eliminating these degree-2 terms.

        Step 2. Call solveSingleVariable on equations that only contain either (c0, s0) or (c1, s1).

        Step 3. Find groups that each have two or more equations that satisfy each of the following two cases.
                Case 1: Equations where all monoms but one involve one of c0, s0, c1, s1. (useconic = False)
                Case 2: Equations where all monoms but one involve only one set of (c0, s0) or (c1, s1). (useconic = True)
        """
        ...
        pairwisevareqns = [Poly(eqnsym[0].subs(pairwisesubs), *pairwisevars) \
                      for rank, eqnsym in eqnsyms if rank < 4*maxcomplexity] # complexity < 200
        for eq in pairwisevareqns:
        ...
        pairwisevareqnsdict = [eq.as_dict() for eq in pairwisevareqns]
        neweqnscount = 20
        for i in range(6):
            monom = [0, 0, 0, 0, 0, 0] # each position corresponds to each pairwise variable
            monom[i] = 1
            m = tuple(monom)
            for j1, eq0 in enumerate(pairwisevareqns):
                eq0dict = pairwisevareqnsdict[j1]
                eq0value = eq0dict.get(m, S.Zero)
                if eq0value == S.Zero:
                    continue
                for j2, eq1 in enumerate(pairwisevareqns[j1+1:]):
                    j2 += j1+1
                    eq1dict = pairwisevareqnsdict[j2]
                    ...
                    if eq == S.Zero:
                        continue
                    if eq.has(*pairwisevars) or self.codeComplexity(eq) > maxcomplexity: # 50
        ...
        systemofequations = []
        for pairwisevareqn in pairwisevareqns:
            if pairwisevareqn.has(s0c0, s1c1) or \
               not all([sum(m) <= 1 for m in pairwisevareqn.monoms()]):
                continue
            arr = [S.Zero]*5
            for m, c in pairwisevareqn.terms():
                # here sum(m) = 0 or 1
            ...
            for eqs in combinations(systemofequations, 4):
                ...
                for i, arr in enumerate(eqs):
                    for j in range(4):
                        A[i,j] = arr[j]
                    b[i] = -arr[4]
                detA = self.det_bareis(A, *(self.pvars+c0s0c1s1)).subs(allsymbols)
                """
        ...
        for ivar in (0, 1):
            ...
            for rank, eq in orgeqns:
                ...
                if sum(p.degree_list()) == 1 and sum(p.LM()) == 1: # TGN: this is TOO strong
                    polyunknown.append((rank, p))
            if len(polyunknown) > 0:
                break
            
        if len(polyunknown) == 0 and len(orgeqns) >= 2:
            addedeqs = eqns[:]
            polyeqs = []
            for ivar in (0, 1):
        ...
        if len(rawsolutions) > 0:
            ...
            log.info('[SOLVE %i] solvePairVariables (M1) returns %i solution(s) for %r, %r', \
                     self._solutionStackCounter, len(rawsolutions), var0, var1)
            return solutions
        ...
        if len(goodgroups) == 0:
            ...
            if len(goodgroups) == 0:
                try:
                    ...
                    log.info('[SOLVE %i] Cannot use solvePairVariablesHalfAngle to solve for %r, %r: %s', \
                             self._solutionStackCounter, var0, var1, e)
                finally:
                    self._dec_solutionStackCounter()
                ...
                for rank, eq in neweqns:
                    ...
                    if not iscoupled:
                        lrhseqns.append([p-p.TC(), Poly(-p.TC(), c1, s1)])
                if len(lrhseqns) > 0:
                    for ivar in (0, 1):
                        # eq[0] is polynomial in c0, s0; eq[1] in c1, s1
                        jvar = 1-ivar
                        lineareqs = [eq for eq in lrhseqns if sum(eq[ivar].LM())==1]
                        ...
                        for paireq0, paireq1 in combinations(lineareqs, 2):
                            ...
                            if disc == S.Zero:
                                continue
                            othereq0 = paireq0[jvar].as_expr() - eq0.TC()
                            othereq1 = paireq1[jvar].as_expr() - eq1.TC()
                            csol =  eq1sval*othereq0 - eq0sval*othereq1 # c*disc
                            ssol = -eq1cval*othereq0 + eq0cval*othereq1 # s*disc

                            # eliminate (c0, s0) when ivar==0 and eliminate (c1, s1) when ivar==1
                            ...
                            for m, c in ptotal_cos.terms():
                                if m[1] > 0:
                                    ...
                                    ptotal_cos = ptotal_cos.sub(Poly.from_dict({(m[0], 1):c}, *polysymbols))
                ...
                log.info('[SOLVE %i] solvePairVariables tries solveSingleVariableLinearly for %r', \
                         self._solutionStackCounter, var0)
                ...
                if len(lineareqs) > 0:
                    try:
                        log.info('[SOLVE %i] solvePairVariables tries solveHighDegreeEquationsHalfAngle for %r', \
                                self._solutionStackCounter, var1)
                        self._inc_solutionStackCounter()
                        return [self.solveHighDegreeEquationsHalfAngle(lineareqs, var1)]
                    except self.CannotSolveError, e:
                        log.warn('%s', e)
                        log.info('[SOLVE %i] Cannot use solveHighDegreeEquationsHalfAngle for %r', \
                                 self._solutionStackCounter, var1)
                    finally:
                        self._dec_solutionStackCounter()
        ...
        assert(not useconic or varindex in (0, 2))
        ...
        for i in (0, 1):
            term = [(m, c) for m, c in eqs[i].terms() \
                    if sum(m) > ((m[varindex] + m[varindex+1]) if useconic else m[varindex] )]
            if len(term) > 0:
        ...
        if domagicsquare:
            ...
            """
            checkforzeros = [self.removecommonexprs(denom) for denom in denoms if denom != S.One]
            denomlcm = lcm(denoms)
            finaleq = simplify(finaleq * denomlcm**2)
            ...
            p0 = Poly(simpleterms[0], c0s0c1s1[varindex], c0s0c1s1[varindex+1])
            p1 = Poly(simpleterms[1], c0s0c1s1[varindex], c0s0c1s1[varindex+1])
        ...
        newc0s0c1s1 = [v for v in c0s0c1s1 if v != unknownvar]
        ...
        try:
            log.info('[SOLVE %i] solvePairVariables tries solvePairVariablesHalfAngle', \
                     self._solutionStackCounter)
            return self.solvePairVariablesHalfAngle(raweqns, var0, var1, othersolvedvars)
        except self.CannotSolveError:
            pass
        ...
        if useconic:
            ...
            log.info('[SOLVE %i] solvePairVariables tries solveHighDegreeEquationsHalfAngle', \
                     self._solutionStackCounter)
            solution = self.solveHighDegreeEquationsHalfAngle([finaleq], var)
            solution.checkforzeros += checkforzeros
            log.info('[SOLVE %i] solvePairVariables returns a solution', self._solutionStackCounter)
            return [solution]

        # TGN: seems never reach here
        exec(ipython_str, globals(), locals())
        
        # now that everything is with respect to one variable, simplify and solve the equation
        eqnew, syms = self.groupTerms(finaleq, c0s0c1s1, symbolgen)
        allsymbols += syms
        ...
        if solutions:
            ...
            log.info('[SOLVE %i] solvePairVariables returns a solution in %s', \
                     self._solutionStackCounter, 
                     'jointevalcos' if varindex%2 == 0 else 'jointevalsin')
            return [solversolution]
        
        log.info('[SOLVE %i] solvePairVariables tries solvePairVariablesHalfAngle', self._solutionStackCounter)
        return self.solvePairVariablesHalfAngle(raweqns, var0, var1, othersolvedvars)
    ...
    def solvePairVariablesHalfAngle(self, raweqns, var0, var1, \
        ...
        htvar0, htvar1 = varsym0.htvar, varsym1.htvar
        # numerators
        halftannum   = [1-htvar0**2, 2*htvar0, \
                        1-htvar1**2, 2*htvar1]
        # denominators
        halftandenom = [1+htvar0**2, 1+htvar1**2]

        # half tangent substitutions
        halftansubs = [(cvar0, halftannum[0] / halftandenom[0]), \
                       (svar0, halftannum[1] / halftandenom[0]), \
                       (cvar1, halftannum[2] / halftandenom[1]), \
                       (svar1, halftannum[3] / halftandenom[1])  ]
        ...
        for eq in raweqns:
            ...
            for monoms, c in peq.terms():
                term = c * self.prod(halftannum[i]**monoms[i] for i in range(4)) * \
                       halftandenom[0]**(maxdenom[0]-monoms[0]-monoms[1])   * \
                       halftandenom[1]**(maxdenom[1]-monoms[2]-monoms[3])
                # too big, so don't simplify?
                eqnew += simplify(term) if self.codeComplexity(term.expand())<450 else term
            polyeq = Poly(eqnew, htvar0, htvar1)
            
            if polyeq.TC() == S.Zero:
                log.info('Trailing coefficient is 0, i.e. for power (0,0)')
                # might be able to divide out variables?
                minmonoms = [min(monom[i] for monom in polyeq.monoms()) for i in (0,1)]
                if minmonoms[0]>0 or minmonoms[1]>0:
                    newpolyeq = sum(Poly.from_dict({tuple([m[0]-minmonoms[0], \
                                                           m[1]-minmonoms[1]]):c}, \
                                                   htvar0, htvar1) \
        ...
        eqtosolve = self.sortComplexity([eq.as_expr() for eq in polyeqs if not eq.has(htvar1)])
        if len(eqtosolve)>0:
            try:
                log.info('[SOLVE %i] solvePairVariablesHalfAngle tries solveSingleVariable for %r', \
                         self._solutionStackCounter, var0)
                self._inc_solutionStackCounter()
                solution = self.solveSingleVariable(eqtosolve, var0, \
                                                    othersolvedvars, unknownvars = [])
                log.info('[SOLVE %i] solvePairVariablesHalfAngle returns solution for %r', \
                         self._solutionStackCounter, var0)
                return solution
            except self.CannotSolveError:
                log.info('[SOLVE %i] Cannot use solveSingleVariable for %r', \
                         self._solutionStackCounter, var0)
            finally:
                self._dec_solutionStackCounter()

        eqtosolve = self.sortComplexity([eq.as_expr() for eq in polyeqs if not eq.has(htvar0)])
        if len(eqtosolve)>0:
            try:
                log.info('[SOLVE %i] solvePairVariablesHalfAngle tries solveSingleVariable for %r', \
                         self._solutionStackCounter, var1)
                self._inc_solutionStackCounter()
                solution = self.solveSingleVariable(eqtosolve, var1, \
                                                    othersolvedvars, unknownvars = [])
                log.info('[SOLVE %i] solvePairVariablesHalfAngle returns solution for %r', \
                         self._solutionStackCounter, var1)
                return solution
        ...
        if pfinals is not None:
            ...
            for newreducedeqs in combinations(polyeqs, 2):
                try:
                    ...
                    tempsubs2 = [(gconst, expr.as_expr()) for gconst, expr in tempsubs]
                    det = Poly(dettemp.subs(tempsubs2), leftvar)

                    """
                    for arg in dettemp.args:
                        eqmuls = [Poly(arg2.subs(tempsubs), leftvar) for arg2 in arg.args]

                        prod1 = expand(arg.subs(tempsubs2).as_expr())
                        if sum(eqmuls[0].degree_list()) == 0:
                            eq = eqmuls.pop(0)
                            eqmuls[0] = eqmuls[0]*eq
                        ...
                        while len(eqmuls) > 1:
                            ...
                            while ioffset < len(eqmuls)-1:
                                ...
                                ioffset += 2
                            eqmuls = eqmuls2
                        checkifzero = expand(eqmuls[0].as_expr()) - prod1
                        assert(checkifzero==S.Zero)
                        eqadds.append(eqmuls[0])
...
class AST:
    ...
    class SolverPolynomialRoots(SolverBase):
        ...
        checkforzeros        = []
        postcheckforzeros    = [] # fail if any zero
        postcheckfornonzeros = [] # fail if any nonzero
        postcheckforNumDenom = None # list of (A,B) pairs where Ax=B was used. Fail if A==0 && B!=0
        postcheckforrange    = [] # checks that value is within [-1,1]
        dictequations        = None
    ...
    class SolverFunction(SolverBase):
        """
        Used to generate code inside a function to "make compiled code smaller". 

        Called by 
        solveFullIK_TranslationDirection5D
        solve5DIntersectingAxes
        solveFullIK_6DGeneral
    ...
    class SolverIKChainAxisAngle(SolverBase):
        """
        Called by solveFullIK_TranslationAxisAngle4D.
        """
        solvejointvars = None
        freejointvars  = None
        jointtree      = None
